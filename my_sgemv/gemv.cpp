...
int main(int argc, char *argv[])
{
    ...
    //A * x + 0*y
    cout<<"Running..."<<endl;
    fb.sgemv("sgemv", FBLAS_NO_TRANSPOSED, n, m, alpha, fpga_A, m, fpga_x, 1, beta, fpga_y, 1);
    queue.enqueueReadBuffer(fpga_y,CL_TRUE,0,m*sizeof(float),res);
    const double end_time = aocl_utils::getCurrentTimestamp();
#else
    std::vector<cl::Event> gemv_event;
    cl::Event e;
    fb.sgemv("sgemv", FBLAS_NO_TRANSPOSED, n, m, alpha, fpga_A, n*m, fpga_x, 1, beta, fpga_y, 1, &e,nullptr);
